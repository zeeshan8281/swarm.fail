...
// Minimum swarm size to be ranked. It IS a swarm benchmark: the theoretical
// optimum of agents×steps is a single agent sweeping each cell once (≈ FLOOR),
// which no swarm can beat. Below this floor a submission is unranked (FAIL), so
// solo/tiny "sweepers" can't win on a technicality.
