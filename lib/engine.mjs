...
function simulate(step, n, seed) {
  ...
  function tick() {
    ...
    for (var i = 0; i < agents.length; i++) {
      ...
      // Read the move inside the guard too: dx/dy may be getters, and a throwing
      // one used to escape tick() entirely and take the whole run (or the browser
      // arena's animation loop) with it.
      var rdx = 0, rdy = 0, rmark = 0;
      try {
        var mv = step(a, env, rngs[i]);
        if (mv) { rdx = mv.dx; rdy = mv.dy; rmark = mv.mark; }
      } catch (e) { rdx = 0; rdy = 0; rmark = 0; }
      // A move must be exactly -1, 0 or 1. Math.sign returns NaN for a string or
      // an object, and NaN coordinates slip past every bounds check below:
      // wall[NaN] and covered[NaN] are undefined, so the agent teleports out of
      // the grid AND re-counts itself as new coverage on every tick. Returning
      // {dx:"x"} used to "cover" 96% of a map while standing still, and scored
      // below the provable floor. Anything that isn't ±1 is a no-op.
      var dx = Math.sign(rdx || 0), dy = Math.sign(rdy || 0);
      if (dx !== 1 && dx !== -1) dx = 0;
      if (dy !== 1 && dy !== -1) dy = 0;
      if (dx && dy) dy = 0;
      ...
      var m = rmark; if (m > 0) deposit[k] += m > 1 ? 1 : m;
    }
